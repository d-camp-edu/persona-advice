...
/** 계열 조합(집합)이 정확히 일치하는 첫 약제를 찾는다 — 시드 수치 변화에 견고. */
function byClasses(...cls: string[]): Medication {
  const want = [...cls].sort().join('|');
  const m = meds.find((x) => !x.isNotDrug && [...x.classes].sort().join('|') === want);
  if (!m) throw new Error(`no med with classes {${cls.join(', ')}}`);
  return m;
}

const dppSingle = byClasses('dc_dpp4');
const glp1 = byClasses('dc_glp1');
const metSingle = byClasses('dc_met');
const sglt2Single = byClasses('dc_sglt2');
const tzdSingle = byClasses('dc_tzd');
const dppMet = byClasses('dc_dpp4', 'dc_met');
...
describe('checkDeductions — DPP-4i + GLP-1 RA 병용 금지', () => {
  it('단독 처방으론 삭감 없음', () => {
    const res = checkDeductions([dppSingle], [], 7.0, NO_RULES, NO_ALLOW, settings);
  ...
  it('상위 집합 허용 조합: [메트+SGLT2i+DPP4i] 등록 시 부분 집합 [SGLT2i+DPP4i] 삭감 규칙 면제', () => {
    ...
    const res = checkDeductions([metSingle, sglt2Single, dppSingle], [], 7.0, rules, allow, settings);
  ...
  it('허용 조합 없으면 [SGLT2i+DPP4i] 삭감 규칙은 그대로 발동', () => {
    ...
    const res = checkDeductions([metSingle, sglt2Single, dppSingle], [], 7.0, rules, NO_ALLOW, settings);
  ...
  it('처방이 허용 조합을 다 포함하지 않으면 면제 안 됨 (허용에 없는 GLP-1 RA 추가)', () => {
    ...
    const res = checkDeductions([sglt2Single, dppSingle, glp1], [], 7.0, rules, allow, settings);
    expect(res).toContain('SGLT2i+DPP4i 병용 삭감!');
  });
});
...
describe('checkDeductions — E11 4규칙', () => {
  it('규칙 A: HbA1c < 6.5%면 삭감', () => {
    const res = checkDeductions([metSingle], ['E11'], 6.0, NO_RULES, NO_ALLOW, settings);
    expect(res).toContain('당뇨(E11) 초기 HbA1c 6.5% 미만 처방 삭감!');
  });

  it('규칙 B: HbA1c >= 6.5% + 단일계열 + 메트포르민 미포함 → 삭감 (DPP-4i 단독)', () => {
    const res = checkDeductions([dppSingle], ['E11'], 7.0, NO_RULES, NO_ALLOW, settings);
...
describe('checkDeductions — 계열 수 초과 / 동일 계열 중복', () => {
  it('4제 이상(메트+SGLT2i+DPP4i+TZD) 병용 시 삭감', () => {
    const res = checkDeductions(
      [metSingle, sglt2Single, dppSingle, tzdSingle],
      [],
      8.0,
      NO_RULES,
      NO_ALLOW,
  ...
  it('3제 이하면 계열 수 초과 삭감 없음', () => {
    const res = checkDeductions(
      [metSingle, sglt2Single, dppSingle],
      [],
      8.0,
      NO_RULES,
      NO_ALLOW,
  ...
  it('복합제(메트+DPP-4i)와 단일 메트포르민 병용 → 동일 계열 중복 삭감', () => {
    ...
    const res = checkDeductions([dppMet, metSingle], [], 8.0, NO_RULES, NO_ALLOW, settings);
    expect(res).toContain('동일 계열 중복 처방 삭감!');
  });

  it('계열이 겹치지 않으면 중복 삭감 없음', () => {
    const res = checkDeductions([metSingle, sglt2Single], [], 8.0, NO_RULES, NO_ALLOW, settings);
